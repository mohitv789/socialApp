...
// reel-dialog.component.ts (modified)
import {
  AfterViewInit,
  Component,
  ElementRef,
...
export class ReelDialogComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // --- NEW: query slide images and close button ---
  @ViewChildren('slideImg') slideImgEls!: QueryList<ElementRef<HTMLImageElement>>;
  ...
  ngAfterViewInit(): void {
    ...
    setTimeout(() => {
      ...
      // AFTER the view is stable, attempt autofocus:
      // prefer the current slide image; fallback to close button
      setTimeout(() => {
  ...
  // Attempt to focus the active slide image; fallback to close button
  private focusInitialElement(): void {
    try {
      // QueryList might not be ready instantly; ensure it's available
      const imgs = this.slideImgEls?.toArray() || [];
      const targetImg = imgs[this.currentIndex];

      if (targetImg && targetImg.nativeElement) {
        // ensure image element is focusable (we add tabindex in template)
        targetImg.nativeElement.focus();
        return;
      }
      ...
      // swallow any errors — focusing is best-effort
      console.warn('focusInitialElement error', e);
    }
  }
  ...
  prevSlide() {
    ...
    this.currentIndex =
      (this.currentIndex - 1 + this.reelimages.length) % this.reelimages.length;
    // refocus current slide after manual navigation
